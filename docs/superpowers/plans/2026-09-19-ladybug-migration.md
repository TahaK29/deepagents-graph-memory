...
- Windows requires external OpenSSL 3 DLLs; importing Python's `ssl` module does not satisfy Ladybug's native dependency. CI provisions the current vendor runtime with its published checksum and makes its two DLLs available in the Windows system directory, matching the documented installer option for customers. Native loading and extension provisioning now pass; full Windows results remain pending.
- The final 0.20.3 wheel and source distribution build successfully. An isolated wheel installation has no Kuzu, declares the exact engine pin, and passes all 29 native integration, FTS, import, and persistence checks.
