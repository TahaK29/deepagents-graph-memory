# - Keeps graph items and their connections in LadybugDB, and finds them again when asked.
# - Keeps each workspace's context separate when a scope is set.
# - Tests: test_ladybug_integration.py checks saving and reading a connection;
#        test_ladybug_search.py checks search, and test_scope.py checks separation.

"""LadybugDB adapter for graph memory."""
...
_disk_lock = RLock()
_open_disk_stores: WeakValueDictionary[tuple[object, ...], LadybugGraphStore] = WeakValueDictionary()


class _LadybugGraph:
    """Minimal query and schema adapter over a `ladybug.Connection`.

    Uses the native connection for query execution and schema reflection.
    """

    def __init__(self, database: Any) -> None:
        self.conn = ladybug.Connection(database)
        self.schema = ""
        try:
            self.refresh_schema()
    ...
    def refresh_schema(self) -> None:
        ...
        `SHOW_CONNECTION`) rather than ladybug's private `_get_*` methods, so the
...
def _locked(method: Any) -> Any:
    @wraps(method)
    def wrapper(self: LadybugGraphStore, *args: Any, **kwargs: Any) -> Any:
        with self._lock:
            self._require_open()
...
class LadybugGraphStore:
    """Internal adapter for the native LadybugDB graph connection."""

    def __init__(self, graph: Any, *, database: Any = None) -> None:
        """Initialize the adapter.

        Args:
            graph: Query and schema adapter over a LadybugDB connection.
    ...
    def transaction(self) -> Any:
        """Serialize and atomically group writes on the shared LadybugDB connection."""
        with self._lock:
            self._require_open()
                ...
                if outermost:
                    if self._rollback_only:
                        try:
                            ...
                            pass  # Preserve the original write failure if LadybugDB already aborted.
    ...
    @classmethod
    def memory(cls) -> LadybugGraphStore:
        """Create an in-memory LadybugDB graph store."""
        return cls._open(":memory:")

    @classmethod
    def disk(cls, path: str | Path) -> LadybugGraphStore:
        ...
        try:
            with _disk_lock:
                ...
                if any(key in _open_disk_stores for key in keys):
                    msg = f"LadybugDB graph at path {filename} is already open in this process; reuse the existing backend.store."
                    raise GraphMemoryConfigurationError(msg)
    ...
    @classmethod
    def _open(cls, path: str) -> LadybugGraphStore:
        try:
        ...
        try:
            ...
            database.close()
            msg = f"Could not initialize LadybugDB graph at path {path!r}: {exc}"
            raise GraphMemoryConfigurationError(msg) from exc
        except BaseException:
            database.close()
            raise
    ...
    def close(self) -> None:
        """Release an owned LadybugDB connection and database."""
        with self._lock:
            if self._closed:
                return
    ...
    def search(self, query: str, *, scope_key: str | None = None, limit: int = 20) -> SearchResult:
        ...
        if self._transaction_depth:
            msg = "search requires LadybugDB auto transaction mode."
            raise GraphMemoryConfigurationError(msg)
    ...
    def _search_fts(self, query: str, *, scope_key: str | None, limit: int) -> list[tuple[float, SearchItem]]:
        ...
        for label in self._labels():
            ...
            try:
                ...
                msg = f"LadybugDB full-text search failed for label {label!r}. Ensure the LadybugDB fts extension is available."
                raise GraphMemoryConfigurationError(msg) from exc
            for row in rows:
    ...
    def _ensure_fts_index(self, label: str) -> bool:
        ...
        try:
            ...
            if "already" not in message and "exist" not in message:
                msg = f"LadybugDB full-text search index creation failed for label {label!r}."
                raise GraphMemoryConfigurationError(msg) from exc
    ...
    def _query(self, query: str, params: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if self._transaction_depth:
                self._rollback_only = True
            msg = f"LadybugDB graph query failed: {exc}"
            raise GraphMemoryConfigurationError(msg) from exc
